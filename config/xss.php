...
return array(
	...
	'escape_html' 				=> '/f_escape_js|f_escape_xml|escape:html|escape/',		//html转义函数配置
	'escape_event' 				=> '/f_escape_event/',					//标签事件属性值场景转义函数配置
	'escape_data' 				=> '/f_escape_data/',					//json数据转义配置
	'escape_path' 				=> '/f_escape_path|f_escape_xml|escape:url/',				//url属性里转义
	...
	//全局白名单，变量名与正则匹配则可信，无需转义
 	//安全变量列表，必须是正则，不包含$ 
	'XSS_SAFE_VAR'          => array(
        '/(?:Url|Enc|Num|flag)$/ies',
        '/^(?:is|bdUid|user|cms|qbMenu|zhima|b|c)/ies'
    ), 
	//精确白名单，某特定文件某个变量是完全可信的
	'file_safe_var'			=> array(
	)	//filename为相对路径（非中文），$varname为变量，表明filename下的所有$varname都是白名单
