...
class FISPagelet {
    ...
    const CSS_INLINE_LINKS_HOOK = '<--[FIS_CSS_INLINE_LINKS_HOOK]-->';

    // 新增superframe静态文件的hook, by jinzhan@baidu.com
    const CSS_INLINE_LINKS_SF_HOOK = '<--[FIS_CSS_INLINE_LINKS_SF_HOOK]-->';
    const JS_SCRIPT_SF_HOOK = '<!--[FIS_JS_SCRIPT_SF_HOOK]-->';
    ...
    // 定义superframe
    static public $is_superframe = false;
    ...
    static public function init($default_mode) {
        ...
        // superframe环境 TODO 上线后删掉 $_GET
        // if(defined("ZD_SUPERFRAME") || $_GET['sf_is_greedy']) {
        if(defined("ZD_SUPERFRAME")) {
            self::$is_superframe = true;
        }
    ...
    public static function jsHookSf() {
        return self::JS_SCRIPT_SF_HOOK;
    }
            ...
            if ($context['hit']) {
                ...
                if (!$has_parent) {
                    ...
                    if ($widget_mode == self::MODE_BIGRENDER) {
                        ...
                        $out .= $html;
                        if ($widget_script) {
                            if(self::$is_superframe) {
                                $out .= '<script type="text/javascript">';
                                $out .= '!function(F){var require = _zd_global_.require;';
                                $out .= implode('', $widget_script);
                                $out .= '}(_zd_global_.F);';
                                $out .= '</script>';
                            }else{
    ...
    static public function renderStatic($html, $arr, $clean_hook = false) {
        if (!empty($arr)) {
            ...
            //替换
            if(strpos($html, self::CSS_INLINE_LINKS_HOOK)){
                $html = str_replace(self::CSS_INLINE_LINKS_HOOK, '<textarea id="g_fis_css" style="display:none;">'.$code.'</textarea>' . self::CSS_INLINE_LINKS_HOOK, $html);
            }else{
                $html = str_replace(self::CSS_LINKS_HOOK, $code . self::CSS_LINKS_HOOK, $html);
            }
        }
        if ($clean_hook) {
            $html = str_replace(array(self::CSS_LINKS_HOOK, self::JS_SCRIPT_HOOK, self::CSS_INLINE_LINKS_HOOK), '', $html);
        }
        return $html;
    }
    ...
    /**
     * 渲染superframe下的静态资源, 
     * _zd_global_ 是知道下的superframe全局变量
     * by jinzhan@baidu.com
     * @param $html
     * @param $arr
     * @param bool $clean_hook
     * @return mixed
     */
    static public function renderSuperFrameStatic($html, $arr, $clean_hook = false) {
        if (!empty($arr)) {
            $code = '';
            $code .= '<script type="text/javascript">';
            $resource_map = $arr['async'];
            ...
            if ($loadModJs) {
                if ($resource_map) {
                    $code .= '_zd_global_.require.resourceMap('.json_encode($resource_map).');';
                }
                $code .= PHP_EOL;
                $code .= '_zd_global_.loader';
                ...
                foreach ($arr['js'] as $js) {
                    ...
                    // $code .= '<script type="text/javascript" src="' . self::getCdn() . $js . '"></script>';
                    $code .= '.add("'. self::getCdn() . $js.'", !0)';
                }
                $code = substr($code, 0, -1);
                $code .= ', function(){';
                $code .= 'console.log("ZD_SF Starts!");';
            }

            if (!empty($arr['script'])) {
                foreach ($arr['script'] as $inner_script) {
                    $code .= '!function(F){'. PHP_EOL . 'var require = _zd_global_.require;'. PHP_EOL .$inner_script.'}(_zd_global_.F);'. PHP_EOL;
                    // $code .= '!function(F){'. PHP_EOL . $inner_script.'}(_zd_global_.F);'. PHP_EOL;
                }
            }
            ...
            $html = str_replace(self::JS_SCRIPT_SF_HOOK, $code . self::JS_SCRIPT_SF_HOOK, $html);
            ...
            //替换
            $html = str_replace(self::CSS_INLINE_LINKS_SF_HOOK, $code . self::CSS_INLINE_LINKS_SF_HOOK, $html);
        }
        if ($clean_hook) {
            $html = str_replace(array(self::CSS_LINKS_HOOK, self::JS_SCRIPT_HOOK, self::JS_SCRIPT_SF_HOOK, self::CSS_INLINE_LINKS_SF_HOOK), '', $html);
        }
    ...
    static function merge_resource(array $array1, array $array2) {
        ...
        foreach ($res as $key => $val) {
            ...
            if ($key != 'async') {
                ...
                $merged = array(
                    'res' => array_merge((array)$array1['async']['res'], (array)$array2['async']['res']),
                    'pkg' => array_merge((array)$array1['async']['pkg'], (array)$array2['async']['pkg'])
                );
            }
            //合并收集
            $array1[$key] = $merged;
    ...
    static public function display($html) {
        ...
        switch($mode) {
            case self::MODE_NOSCRIPT:
                ...
                $html = self::$is_superframe ? self::renderSuperFrameStatic(
                    $html,
                    $all_static,
                    true
                ) : self::renderStatic(
