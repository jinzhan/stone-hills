...
class FISResource {
    ...
    public static function register($strNamespace, $smarty){
        ...
        // 添加https-map的逻辑，by jinzhan
        if($strNamespace === '__global__'){
            if(isset($_SERVER['HTTP_X_SSL_HEADER']) && $_SERVER['HTTP_X_SSL_HEADER']){
                if(!defined("ZD_SUPERFRAME")) {
                    $strMapName = 'https-map';
                }else{
                    // superframe下的样式兼容
                    $strMapName = 'sf-map';
                }
            ...
            if(isset($_SERVER['HTTP_X_SSL_HEADER']) && $_SERVER['HTTP_X_SSL_HEADER']){
                if(!defined("ZD_SUPERFRAME")) {
                    $strMapName = $strNamespace . '-https-map';
                }else{
                    // superframe下的样式兼容
                    $strMapName = $strNamespace . '-sf-map';
                }
            }else{
               $strMapName = $strNamespace . '-map';
            }
